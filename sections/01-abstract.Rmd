...
Therefore, we here investigated the capture efficiency of the CONTRAfluran^TM^ absorbers for sevoflurane
in routine anaesthesia practice.

**Design**: Prospective, investigator-initiated, observational study.

**Setting**:
`r nrow(agc)` consecutive CONTRAfluran^TM^ absorbers in
two operating rooms (ORs) at a university hospital from
`r format(min(periods$Start), "%B %Y")` to
`r format(max(periods$End), "%B %Y")`.

**Participants**
A total of `r sum(nCasesSev)` balanced anaesthesia cases.
