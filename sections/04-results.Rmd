...
We monitored a total of `r sum(nCases)` anaesthesia cases
...
for a total anaesthesia duration of `r sprintf("%.0f", sum(agc$TotalDurationCasesSev) / 60)` h.
...
Sevoflurane attributed `r .pct(RatioSevo, prec = ".1")` of the canisters' weight gain.
...
sevoflurane used, in the ratio of patients receiving
VAs and the durations of anaesthesia (Table \@ref(tab:table1)A and B).

While the median weight gain of the canisters did not differ between the two
...
weight gain of each canister, the proportional capture efficiency differed
...
The details of each canister can be found in Supplemental Table S\@ref(tab:tableagc).

## Weight loss

The `r paste0(nrow(agc), " saturated canisters were stored for ", .medLU(agc$DaysStored), " days")`
...
Over the entire storage period we observed a loss of
`r .medLU(agc$LostWeight2)` gram in median (lower and upper quartile) for each
canister and of `r TotalWeightLoss` gram in total.
This corresponds to `r .pct(RatioWeightLoss)` of the canisters' weight gain
(Figure \@ref(fig:plot-weight-lost)).

```{r plot-weight-lost, dev = ifelse_is_docx("tiff", "png"), fig.width = 9, fig.height = 6, echo = FALSE, fig.cap = "Weight loss of the anaesthetic gas canisters during storage. The dots represent the weight loss of each individual anaesthetic gas canister for the days after exhaustion before shipping and desorption. The gray lines connect the same anaesthetic gas canisters measured at the two different time points."}
