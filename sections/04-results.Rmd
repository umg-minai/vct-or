...
During the `r .as.word(ceiling(difftime(max(periods$End), min(periods$Start), units = "weeks")))` week study period we observed in total `r sum(nCases)` anaesthesia cases,
of which `r .pct(sum(nCasesSev) / sum(nCases), prec = ".0")` received volatile
...
used sevoflurane, which is partly due to differences in the ratio of patients receiving
VAs and the differences in the duration of anaesthesia (Table \@ref(tab:table1)A and B).

While median weight gain of the AGCs did not differ between the two
...
In median (lower and upper quartile) the `r paste0(nrow(agc), " AGCs were stored for ", .medLU(agc$DaysStored), " days")`.
